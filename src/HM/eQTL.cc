...
double snp_eQTL::compute_log10_config_BF(int config){
  return log10_weighted_sum(gm[config],grid_wts,grid_size);  
}


void snp_eQTL::print_result(vector<string> & tv){
...
double* gene_eQTL::em_update_grid(int grid_size){
...
double* gene_eQTL::em_update_config(int config_size){
...
void gene_eQTL::compute_posterior(double *config_prior, int config_size){

  // gene level posterior
  post_prob_gene = 1.0 - pow(10,(log10(*Pi0)-log10_lik));
...
void gene_eQTL::print_result(vector<string> &tv){
  
  for(int i=0;i<snpVec.size();i++){
    printf("%12s\t%7.3f\t%9.3f\t\t",gene.c_str(),post_prob_gene,log10_BF);
    snpVec[i].print_result(tv);
    printf("\n");
  }
}
